...
def now_br():
    # Pega a hora atual UTC e tira 3 horas
...
# Ajustado para 4.5MB para alinhar com o limite da Vercel e evitar erro 413
...
class Community(db.Model):
    ...
    created_at = db.Column(db.DateTime, nullable=False, default=now_br) # DATA CORRIGIDA
...
class Post(db.Model):
    ...
    timestamp = db.Column(db.DateTime, nullable=False, default=now_br) # DATA CORRIGIDA
...
class Notification(db.Model):
    ...
    timestamp = db.Column(db.DateTime, index=True, default=now_br) # DATA CORRIGIDA
...
class Aquarium(db.Model):
    ...
    created_at = db.Column(db.DateTime, default=now_br) # DATA CORRIGIDA
    user_id = db.Column(db.Integer, db.ForeignKey('user.id', ondelete='CASCADE'), nullable=False)
...
class ParameterLog(db.Model):
    ...
    date = db.Column(db.DateTime, default=now_br); ph = db.Column(db.Float, nullable=True) # DATA CORRIGIDA
...
class MaintenanceLog(db.Model):
    id = db.Column(db.Integer, primary_key=True); aquarium_id = db.Column(db.Integer, db.ForeignKey('aquarium.id', ondelete='CASCADE'), nullable=False)
    date = db.Column(db.DateTime, default=now_br); maintenance_type = db.Column(db.String(50), nullable=False) # DATA CORRIGIDA
...
class EncyclopediaEntry(db.Model):
    ...
    updated_at = db.Column(db.DateTime, default=now_br, onupdate=now_br) # DATA CORRIGIDA
...
def create_notification(recipient, action, sender=None, post=None, comment=None):
    ...
    if action == 'like_comment' and comment:
        ...
        if existing:
            existing.count += 1; existing.sender_id = sender_id; existing.timestamp = now_br(); db.session.commit(); return # DATA CORRIGIDA
