...
# Inicialização Simplificada (O Flask acha as pastas se estiverem ao lado)
app = Flask(__name__)
...
# --- CORREÇÃO OBRIGATÓRIA PARA NEON DB ---
...
class ParameterLog(db.Model):
    ...
    date = db.Column(db.DateTime, default=datetime.datetime.utcnow); ph = db.Column(db.Float, nullable=True)
...
class MaintenanceLog(db.Model):
    id = db.Column(db.Integer, primary_key=True); aquarium_id = db.Column(db.Integer, db.ForeignKey('aquarium.id', ondelete='CASCADE'), nullable=False)
    date = db.Column(db.DateTime, default=datetime.datetime.utcnow); maintenance_type = db.Column(db.String(50), nullable=False)
...
def create_notification(recipient, action, sender=None, post=None, comment=None):
    ...
    if action == 'like_comment' and comment:
        existing = Notification.query.filter_by(recipient_id=recipient_id, action='like_comment', comment_id=comment.id, is_read=False).first()
        if existing:
            existing.count += 1; existing.sender_id = sender_id; existing.timestamp = datetime.datetime.utcnow(); db.session.commit(); return
...
@limiter.limit("5 per hour") # PROTEÇÃO ANTI-BOT
def register():
    if current_user.is_authenticated: return redirect(url_for('home'))
...
def home():
    ...
    return render_template('index.html', posts=posts, communities=communities, popular_communities=popular, subscribed_communities=current_user.joined_communities[:5])
...
def feed():
    ...
    community_posts = Post.query.filter(Post.community_id.in_([c.id for c in current_user.joined_communities]))
    
    all_posts = followed.union(own).union(community_posts).order_by(Post.timestamp.desc()).paginate(page=page, per_page=10)
    
    if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
        return render_template('post_list.html', posts=all_posts, current_community=None)
    
    return render_template('feed.html', posts=all_posts, communities=Community.query.all(), popular_communities=get_popular_communities())
...
def community_feed(community_slug):
    ...
    if request.method == 'POST':
        ...
        if pic and app.config['CLOUDINARY_API_KEY']:
            try:
                uploaded = cloudinary.uploader.upload(pic, folder="aquanet_posts", resource_type="auto")
    ...
    page = request.args.get('page', 1, type=int)
    posts = Post.query.filter_by(community=community).order_by(Post.timestamp.desc()).paginate(page=page, per_page=10)
    ...
    if request.method == 'POST':
        ...
        if pic and pic.filename != '':
            if app.config['CLOUDINARY_API_KEY']:
                try:
                    uploaded = cloudinary.uploader.upload(pic, folder="profile_pics", transformation=[{'width': 150, 'height': 150, 'crop': 'fill', 'gravity': 'face'}])
...
CATEGORIES_WIKI = ['Peixes', 'Plantas', 'Invertebrados', 'Aquários', 'Equipamentos', 'Doenças', 'Alimentos', 'Química da Água']
@app.route('/encyclopedia')
def encyclopedia():
    return render_template('encyclopedia.html', categories=CATEGORIES_WIKI, popular_communities=get_popular_communities())

@app.route('/encyclopedia/<category>')
def encyclopedia_category(category):
    if category not in CATEGORIES_WIKI:
        return redirect(url_for('encyclopedia'))
    entries = EncyclopediaEntry.query.filter_by(category=category).order_by(EncyclopediaEntry.title).all()
    return render_template('encyclopedia_category.html', category=category, entries=entries, popular_communities=get_popular_communities())
...
def create_entry():
    if request.method == 'POST':
        ...
        img_url = None
        if pic and app.config['CLOUDINARY_API_KEY']:
            try: uploaded = cloudinary.uploader.upload(pic, folder="wiki_images"); img_url = uploaded['secure_url']
            except: pass
...
def edit_entry(slug):
    ...
    if request.method == 'POST':
        ...
        if pic and app.config['CLOUDINARY_API_KEY']:
            try: uploaded = cloudinary.uploader.upload(pic, folder="wiki_images"); entry.image_file = uploaded['secure_url']
            except: pass
...
def aquarium_dashboard(aquarium_id):
    ...
    return render_template('aquarium_dashboard.html', aquarium=aqua, latest_log=ParameterLog.query.filter_by(aquarium_id=aqua.id).order_by(ParameterLog.date.desc()).first(), latest_maint=MaintenanceLog.query.filter_by(aquarium_id=aqua.id).order_by(MaintenanceLog.date.desc()).first(), history_logs=ParameterLog.query.filter_by(aquarium_id=aqua.id).order_by(ParameterLog.date.desc()).all(), history_maint=MaintenanceLog.query.filter_by(aquarium_id=aqua.id).order_by(MaintenanceLog.date.desc()).all(), share_text="📊 Atualização: " + aqua.name, all_communities=Community.query.order_by(Community.name).all(), popular_communities=get_popular_communities())
...
# --- CORREÇÃO DO SCRIPT.JS (CRÍTICO) ---
@app.route('/script.js')
def script_js(): 
    # Usa 'base_dir' corretamente e remove '..'
...
def debug_server():
    ...
    try: conteudo_raiz = os.listdir(path_atual)
    except Exception as e: conteudo_raiz = [str(e)]
    path_templates = os.path.join(path_atual, 'templates')
    if os.path.exists(path_templates):
        try: conteudo_templates = os.listdir(path_templates)
        except Exception as e: conteudo_templates = [f"Erro: {e}"]
    else: conteudo_templates = ["A PASTA NAO EXISTE!"]
    return jsonify({"Onde_Estou": path_atual, "Tem_na_Raiz": conteudo_raiz, "Tem_na_Templates": conteudo_templates})
