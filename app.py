...
def log_parameters(aquarium_id):
    if request.method == 'POST':
        ...
        # --- MOTOR DE DIAGNÓSTICO (Para Retenção de Usuários) ---
        has_critical = False
        
        # Alerta Crítico (Baseado no seu vídeo: Amônia 1.3 / Nitrito 2.36)
        ...
        if not has_critical and (ammonia > 0 or nitrite > 0):
             if ammonia <= 0.25 and nitrite <= 0.5:
                flash("✅ Parâmetros estáveis. Bom trabalho!", 'success')
