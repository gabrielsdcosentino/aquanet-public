...
def log_parameters(aquarium_id):
    if request.method == 'POST':
        ...
        # --- LÓGICA DE DIAGNÓSTICO E RETENÇÃO ---
        mensagens_alerta = []
        
        # Alerta de Amônia (Nível crítico > 0.25)
        if ammonia > 0.25:
            mensagens_alerta.append(f"Amônia em {ammonia} ppm é tóxica! Reduza a comida e faça uma TPA imediata.")
        
        # Alerta de Nitrito (Nível crítico > 0.5)
        if nitrite > 0.5:
            mensagens_alerta.append(f"Nitrito em {nitrite} ppm impede a respiração dos peixes. TPA de emergência recomendada.")

        # Alerta de pH (Baseado em extremos)
        if ph > 8.5:
            mensagens_alerta.append("Seu pH está muito alcalino. Verifique se há pedras calcárias ou excesso de conchas.")
        elif ph < 6.0 and ph > 0:
            mensagens_alerta.append("Seu pH está muito ácido. Isso pode causar um 'crash' biológico.")

        # Exibe os alertas usando o sistema de flash do Flask
        if mensagens_alerta:
            for msg in mensagens_alerta:
                flash(f"⚠️ {msg}", 'danger')
        else:
            flash("Parâmetros registrados! Seu ecossistema parece estável. Bom trabalho!", 'success')
...
def log_parameters(aquarium_id):
    if request.method == 'POST':
        ...
        # --- MOTOR DE DIAGNÓSTICO AQUANET ---
        has_critical = False
        
        # Alerta Crítico de Nitrogênio (Baseado no seu vídeo: Amônia 1.3 / Nitrito 2.36)
        if ammonia > 0.25 or nitrite > 0.5:
            flash(f"🚨 PERIGO! Níveis letais detectados (Amônia: {ammonia} / Nitrito: {nitrite}). Seus peixes correm risco de morte imediata. Faça uma TPA de 50% agora!", 'danger')
            has_critical = True
        
        # Alerta de pH (Guppies, como os do seu vídeo, sofrem com pH muito alto ou baixo)
        if ph > 8.5:
            flash(f"⚠️ Atenção: Seu pH ({ph}) está muito alto. Isso potencializa a toxicidade da amônia.", 'warning')
        elif ph < 6.4 and ph > 0:
            flash(f"⚠️ Atenção: pH ({ph}) muito ácido. Verifique a estabilidade do seu sistema.", 'warning')

        if not has_critical and ammonia <= 0.25 and nitrite <= 0.5:
            flash("✅ Parâmetros registrados com sucesso. Tudo parece sob controle!", 'success')
